...
NEXORA.Views.BOQ = {
  render: function() {
    ...
    var boq = DB.boq_items.filter(function(b) { return b.project_id === pid; });
    var totalAmount = boq.reduce(function(s, b) { return s + (b.amount || 0); }, 0);
    ...
    h += '<div class="card"><div class="card-title"><i class="ti ti-plus-circle"></i> إضافة بند جديد</div>' +
      ...
      '</div>' +
      '<button class="btn btn-primary" onclick="addBOQItem()"><i class="ti ti-device-floppy"></i> إضافة</button>' +
    '</div>';
...
window.addBOQItem = function() {
  ...
  document.getElementById('fBoqItemNo').value = '';
  document.getElementById('fBoqName').value = '';
  document.getElementById('fBoqQty').value = '';
  document.getElementById('fBoqRate').value = '';
  document.getElementById('fBoqAmount').value = '';
  NEXORA.Views.BOQ.render();
};

window.updateExecQty = function(id, val) {
  var DB = NEXORA.DB;
  var b = DB.boq_items.find(function(x) { return x.id === id; });
  if (!b) return;
  var q = parseFloat(val) || 0;
  b.executed_qty = q;
  b.executed_amount = q * (b.unit_rate || 0);
  if (b.executed_qty >= b.quantity && b.quantity > 0) b.status = 'done';
  else if (b.executed_qty > 0) b.status = 'in_progress';
...
window.editBOQItem = function(id) {
  var DB = NEXORA.DB;
  var b = DB.boq_items.find(function(x) { return x.id === id; });
  if (!b) return;
  ...
  b.quantity = newQty;
  b.unit_rate = newRate;
  b.amount = newQty * newRate;
  b.executed_amount = (b.executed_qty || 0) * newRate;
  DB.save();
  if (typeof showToast === 'function') showToast('تم تعديل البند', 'success');
  NEXORA.Views.BOQ.render();
};

window.deleteBOQItem = function(id) {
  if (!confirm('هل أنت متأكد من حذف هذا البند؟')) return;
  var DB = NEXORA.DB;
  DB.boq_items = DB.boq_items.filter(function(x) { return x.id !== id; });
  DB.save();
